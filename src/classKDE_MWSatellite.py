...
class KDE_MWSatellite(object):
    ...
    def fftconvolve_boundary_adjust(self, kernel: np.ndarray) -> np.ndarray:
        ...
        """
        conv = fftconvolve(self.hist2d, kernel, mode='same')
        mask2d = np.ones(self.hist2d.shape)
    ...
    def overdensity(self, sigma: float) -> np.ndarray:
        """ Convolved overdensity map with a Gaussian kernel size sigma """
        ...
        return  self.fftconvolve_boundary_adjust(kernel)

    def get_sig_gaussian(self, od_1: np.ndarray, od_2: np.ndarray,
                         sigma1: float, sigma2: float) -> np.ndarray:
        """ Get a significance map using the 2-gaussian kernel
    ...
    def poisson_inner_number_count(self,
        ...
        conv = self.fftconvolve_boundary_adjust(kernel / norm_kernel)
        return  conv * norm_kernel, norm_kernel

    def poisson_outer_expected_background(self,
            sigma_in: float, sigma_out: float) -> Tuple[np.ndarray, float]:
